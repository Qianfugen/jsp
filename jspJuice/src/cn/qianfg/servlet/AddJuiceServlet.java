...
@WebServlet("/addJuiceServlet")
public class AddJuiceServlet extends HttpServlet {
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (ServletFileUpload.isMultipartContent(request)){//判断是否为多部件
            ...
            int flag=js.addJuice(ju);

            response.sendRedirect("juiceMainServlet");
        }else{
            System.out.println("添加失败,返回添加页面");
            response.sendRedirect("toAddJuiceServlet");
        }
    }
